...
static void build_header(lv_obj_t *parent, lv_obj_t **time_label);
...
void build_header(lv_obj_t *parent, lv_obj_t **time_label) {
...
void home_ui_setup(HomeUI *ui) {
  ...
  build_header(ui->page_one, &ui->time_label_page_one);

  for (uint8_t i = 0; i < 6; ++i) {
    create_icon_button(ui->page_one, kTileOneX[i], kTileOneY[i], kPageOneIcons[i], kPageOneTargets[i]);
  }

  // Page 2 currently exposes the RGB demo as a standalone entry.
  ...
  build_header(ui->page_two, &ui->time_label_page_two);
